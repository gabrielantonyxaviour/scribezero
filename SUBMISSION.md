...
- **0G Compute (TEE)** generates the SOAP note via the 0G **Private** (TEE) router; the response carries a routing proof (provider + chatID + model) that is independently re-checkable on-chain via the broker's `processResponse`. No Compute → no note (it degrades to a clearly-labeled local sample). No Compute → no verifiable inference.
...
## On-chain evidence (live)
- **0G Compute — LIVE.** The SOAP note is generated by `glm-5.2` on a 0G **Private/TEE** provider through `router-api.0g.ai`. A real run: provider `0xF203A388e9E70F09ece38046a6D40a89cf896309`, chatID `94a67252-8921-4cff-811a-9179e725b5b1`, ~35s of real inference inside the attested enclave. Remove the key and the note text falls back to a clearly-labeled local sample — proving Compute is load-bearing, not cosmetic.
- **0G Storage — LIVE.** Uploads confirmed on-chain from `/api/store`:
...
- The `/app` receipt is state-honest: **`0G testnet`** when Compute+Storage are both live (current state), **`0G Storage · live`** if only Storage, **`demo · mock`** if neither.
